...
from data import db_session, task_resource, save_problem_resource, \
    task_name_resource, check_problem_resource
...
from help_function import all_tasks, creation_lesson, calendar_name, \
    students_for_teacher, is_teacher, get_task_names_by_object, all_lessons, \
    get_lesson_names_by_object, creation_course, \
    all_courses, get_lessons_by_course, get_lesson_name_by_id, \
...
def handle_something_go_wrong(e):
    print(e)
    return 'we are so sorry, but something go wrong!', 500
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db_sess.add(user)
        with open('calendars/'+ form.name.data + '.json', 'w') as f:
            data = {'tasks': {'normal': {}, "repetition": [], "hidden_repetition": {}}}
            new_json = json.dump(data, f, indent=4, ensure_ascii=False)
        with open("data/complete_tasks_jsons/" + form.name.data + '.json', 'w') as f:
            data = {}
            new_json = json.dump(data, f, indent=4, ensure_ascii=False)
        db_sess.commit()
        ...
        complete_tasks_json = CompleteTasksJsons(id=student_id, name=form.name.data + '.json')
        db_sess = db_session.create_session()
        db_sess.add(calendar)
        db_sess.add(complete_tasks_json)
        db_sess.commit()
        try:
            my_html = f"""
             <html>
...
@application.route('/teacher_calendar/<int:user_id>/')
@login_required
def teacher_calendar(user_id):
    if is_teacher(current_user.get_id()):
        calendar_title = user_id
        students = students_for_teacher(current_user.get_id())
        students_id = students_ids_for_teacher(current_user.get_id())
        ...
        try:
            data = calendar_data.load_calendar(get_calendar_name_by_id(calendar_title))
        ...
        try:
            ...
            if 'variations_of_answers' in file and len(file['answers']) > 1:
                ...
                if ';' not in file['questions']:
                    ...
                    questions = [i.strip() for i in file['questions'].split(';')]
                variations_of_answers = [i.strip() for i in file['variations_of_answers'].split(';')]
                answers = [i.strip() for i in file['answers'].split(';')]
...
@application.route('/<calendar_id>/<year>/<month>/<day>/<task_id>/delete_task',
...
def student_calendar():
    calendar_title = current_user.id
    current_day, current_month, current_year = GregorianCalendar.current_date()
    month_name = GregorianCalendar.MONTH_NAMES[current_month - 1]
    calendar_data = CalendarData(current_app.config["DATA_FOLDER"],
                                 current_app.config["WEEK_STARTING_DAY"])
    ...
    return render_template('calendar.html',
                           title='Личный кабинет', calendar_id=get_calendar_name_by_id(calendar_title),
                           year=current_year,
                           month=current_month,
                           month_name=month_name,
                           current_year=current_year,
...
def create_lesson(filter_name):
    if is_teacher(current_user.get_id()):
        ...
        if filter_name == 'None':
            ...
            tasks_id = get_task_id_by_object(filter_name)
            return render_template('create_lesson.html', title='Создание уроков',
                                   tasks=tasks_,
                                   objects=config.SUBJECTS,
                                   base_url=current_app.config["BASE_URL"],
                                   object_now=filter_name, form=form, tasks_id=tasks_id)
    else:
        abort(404)
...
def view_task(task):
    variations_of_answers = None
    if len(get_info_about_task_by_id(task)) == 5:
        questions, task_type, variations_of_answers = get_info_about_task_by_id(task)[1:4]
    else:
        questions, task_type, = get_info_about_task_by_id(task)[1:3]
    if ';' in questions:
...
@application.route('/add_course_to_student/<filter_name>/')
def add_course(filter_name):
    if is_teacher(current_user.get_id()):
        form = LessonForm()
        courses = all_courses()
        courses_id = all_courses_id()
        courses_st = "None"
        students_id = students_ids_for_teacher(current_user.id)
...
def adding_course():
    ...
    add_course_to_student(lst, user_id)
    return jsonify({'succes': 'Ok'})
...
def course_in(subject_id):
    lst = get_courses_by_student(current_user.id)
...
def course_lessons_student(course_id):
...
def view_tasks(lesson_id):
    problems = get_all_task_from_lesson(lesson_id)
    task_id = problems[0]
    correct = None
    right_answers = None
    student_answers = None
    user_tasks_json = get_json_name_by_user_id(current_user.id)
    with open("data/complete_tasks_jsons/" + user_tasks_json, 'r', encoding='utf-8') as f:
        data = json.load(f)
    if task_id in data:
        correct = data[task_id]['correct']
        student_answers = [i.strip() for i in data[task_id]['student_answers']]
        right_answers = get_info_about_task_by_id(task_id)[-1]
        if type(right_answers) == int:
            right_answers = str(right_answers)
        if ";" in right_answers:
            right_answers = [i.strip() for i in right_answers.split('; ')]
        else:
            right_answers = [right_answers.strip()]
    return render_template('missions_in_lessons.html', lesson=lesson_id,
                           tasks=problems, base_url=current_app.config["BASE_URL"], task=task_id, correct=correct,
                           correct_answers=right_answers,
                           student_answers=student_answers)


@application.route('/mission_in/<lesson_id>/<task_id>/')
def task_from_student(lesson_id, task_id):
    problems = get_all_task_from_lesson(lesson_id)
    correct = None
    right_answers = None
    student_answers = None
    ...
    if task_id in data:
        ...
        if ";" in right_answers:
            ...
            right_answers = [right_answers.strip()]
    return render_template('one_mission_in_lesson.html', title='ЗАДАЧИ', lesson=lesson_id,
                           tasks=problems, base_url=current_app.config["BASE_URL"], task=task_id, correct=correct,
                           correct_answers=right_answers,
                           student_answers=student_answers)
...
application.jinja_env.globals.update(clever=clever)
if __name__ == '__main__':
    api.add_resource(task_resource.Task, '/save/<calendar_id>/new_task')
    api.add_resource(save_problem_resource.SaveProblem,
                     '/save_problem/<task_type>/')
    api.add_resource(task_name_resource.NameOfTask,
                     '/task_name/<task_id>/')
    api.add_resource(check_problem_resource.CheckProblem,
                     '/check_answer/<user_id>')
